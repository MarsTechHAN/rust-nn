...
use initlizer::zero_initlizer::ZeroInitlizer;
...
fn main() {
    ...
    let mut in_layer = Dense::new("IN", 10usize, 20usize, UniformInitlizer::new(1.0, -1.0), ZeroInitlizer::new());
    let mut relu_layer = ReLU::<2>::new("RELU");

    let in_mat = Array::linspace(0., 10., 10).into_shape((1,10)).unwrap();
